...
| **性能适配** | 分辨率自适应：240p→8K 质量分级（Low/Medium/High/Ultra，按视口面积定级，低级别降阴影/渐变精度）。帧率自适应：rAF vsync 对齐（电视/低端屏自动低帧）+ 耗时预算降帧（慢设备自动跳帧），23fps→显示刷新率上限无缝适配。动态模糊：球沿速度方向的渐变尾迹。 |
| **阶梯淡入** | 页面元素阶梯式淡入（CSS stagger：nth-child 递增 animation-delay），语言无关。 |
| **入场仪式** | 三球初始停在 logo 三球位（锚点 ANCHORS，从原图提取的世界坐标），3s 后随机前往随机网格区域（非固定），到达后沿曲线随机排队，再开始玩耍。状态机：AtLogo → Travel → Queue → Play。 |
| **高速椭圆化** | 球高速运动时沿速度方向椭圆化（速度越快长短轴比越大，很快时明显），与动态模糊尾迹并存。 |
| **logo 定位** | 调试面板导出并写死（2026-08-03）：`left:37.93% top:27.40%`（覆盖默认 50%/50%）。 |
| **拖尾研究** | Google pixel 开机动画的拖尾 = 纯色实心（无渐变、无椭圆混合色）。本实现：等宽实心线段拖尾（与球同宽，无感叹号渐细）。 |
| **Spring 物理（谷歌大学 2026-08 一手来源）** | MDC-Android 官方 MotionTokens：**motionSpringDefaultSpatial = damping 0.9, stiffness 700**（位置动画默认弹簧；damping<1 允许轻微过冲 = 灵动来源）。本实现：球用 spring 追踪链上目标（半隐式欧拉），取代一切缓动曲线——速度天然连续（无卡顿/停顿/突跳），目标线性推进 + spring 提供全部平滑。参数：`SPRING{stiffness:700, damping:0.9}`。 |
| **长页游乐场** | 构想（未实现）：整个页面为长滚动游玩场地（手机不弃游乐设施），不同区域=不同设施；Material You 丝滑响应式承载；区域网格覆盖整页。图形化管理页为妄想，弃。 |
